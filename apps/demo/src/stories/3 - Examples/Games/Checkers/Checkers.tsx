...
import { DndContext, useDraggable, DragEndEvent, DragStartEvent, DragOverlay } from '@dnd-kit/core';
import { Board, Cell, CellProps, Endgame, Piece, PieceProps, Score } from './components';
import styles from './Checkers.module.less';
...
export function Checkers() {
    ...
    const checkCanMove = useCallback(
        function (isOddTurn: boolean, isOddPiece: boolean, movingPiece: PieceProps, moveToX: number, moveToY: number): CanMoveProps {
            if (movingPiece.position) {
                ...
                if (isOddTurn && isOddPiece) {
                    ...
                    if (Math.abs(dx) === 2 && Math.abs(dy) === 2) {
                        ...
                        const potentialEnemyPiece = pieces[potentialEnemyPieceCoordinates.y][potentialEnemyPieceCoordinates.x];
                    ...
                    const canMove = (dy === 1 || dy === 2) && Math.abs(dx) === Math.abs(dy);
                    return { canMove };
    ...
    const handleDragEnd = useCallback(
        function handleDragEnd(event: DragEndEvent) {
            ...
            if (event.over && !potentialExistingPiece) {
                const { canMove, toRemove } = checkCanMove(isOddTurn, movingPiece.odd, movingPiece, cellX, cellY);
            ...
            <DndContext onDragStart={handleDragStart} onDragEnd={handleDragEnd} onDragCancel={handleDragCancel}>
                <div className={styles.Checkers}>
                    {gameEnded ? (
                        ...
                        <Score oddLabel={players[0]} evenLabel={players[1]} oddScore={oddScore} evenScore={evenScore} oddTurn={isOddTurn} />
                    )}
                    <Board size={BOARD_SIZE}>
                        {board.map((row, y) =>
                            row.map((rowCase, x) => {
                                ...
                                if (!piece) {
                                    const canDrop = movingPiece ? checkCanMove(isOddTurn, movingPiece.odd, movingPiece, x, y).canMove : false;

                                    return <Cell key={rowCase.id} validDropLocation={canDrop} {...rowCase} />;
                                }

                                const pieceMarkup = disabled ? <Piece {...piece} disabled /> : <DraggablePiece {...piece} />;
                ...
                <DragOverlay dropAnimation={null}>
                    {movingPiece == null ? null : <Piece odd={movingPiece.odd} clone id={movingPiece.id} />}
                </DragOverlay>
            </DndContext>
        </AnimateSharedLayout>
    );
...
function generateBoard() {
    const board: CellProps[][] = Array.from(Array(BOARD_SIZE), () => new Array(BOARD_SIZE));
