...
export function Pages({ layout }: Props) {
    ...
    const [items, setItems] = useState(() => createRange<UniqueIdentifier>(20, (index) => `${index + 1}`));
    ...
    return (
        ...
        >
            ...
            <DragOverlay dropAnimation={dropAnimation}>{activeId ? <PageOverlay id={activeId} layout={layout} items={items} /> : null}</DragOverlay>
        </DndContext>
    );
...
function PageOverlay({ id, items, ...props }: Omit<PageProps, 'index'> & { items: UniqueIdentifier[] }) {
    ...
    return (
        <Page
            ...
            {...props}
            clone
            insertPosition={isKeyboardSorting && overIndex !== activeIndex ? (overIndex > activeIndex ? Position.After : Position.Before) : undefined}
        />
    );
}
