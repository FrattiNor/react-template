...
class Controller {
    ...
    deleteControl(canvas: fabric.Canvas) {
        ...
        function renderDelIcon(ctx: CanvasRenderingContext2D, left: number, top: number, styleOverride: any, fabricObject: fabric.Object) {
            const size = 24;
            ctx.save();
        ...
        function deleteObject(eventData: MouseEvent, transformData: fabric.Transform): boolean {
            if (transformData.action === 'rotate') return false;
    ...
    enable() {
        // 删除图标
        this.deleteControl(this.editor.canvas);
        // 选中样式
