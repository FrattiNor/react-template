...
const sentences = new Array(10000).fill(true).map(() => faker.lorem.sentence(randomNumber(20, 70)));
...
function ColumnVirtualizerDynamic() {
    ...
    return (
        <>
            <div ref={parentRef} className="List" style={{ width: 400, height: 400, overflowY: 'auto' }}>
                <div
                    style={{
...
function GridVirtualizerDynamic({ columns, data }: { data: string[][]; columns: Column[] }) {
    const parentRef = React.useRef<HTMLDivElement | null>(null);
    ...
    const [before, after] =
        columnItems.length > 0 ? [columnItems[0].start, columnVirtualizer.getTotalSize() - columnItems[columnItems.length - 1].end] : [0, 0];

    return (
        <div ref={parentRef} style={{ overflowY: 'auto', border: '1px solid #c8c8c8' }}>
            <div
                style={{
                    height: virtualizer.getTotalSize(),
                    position: 'relative',
                        ...
                        >
                            ...
                            {columnItems.map((column) => {
                                return (
                                    ...
                                    >
                                        {row.index === 0 ? <div>{columns[column.index].name}</div> : <div>{data[row.index][column.index]}</div>}
                                    </div>
                                );
                            })}
...
function App() {
    ...
    return (
        <div>
            <p>
                These components are using <strong>dynamic</strong> sizes. This means that each element's exact dimensions are unknown when rendered.
                An estimated dimension is used to get an a initial measurement, then this measurement is readjusted on the fly as each element is
                rendered.
            </p>
            <nav>
                <ul>
                    <li>
                        <a onClick={() => setType('/')}>List</a>
                    </li>
                    <li>
                        <a onClick={() => setType('/window-list')}>List - window as scroller</a>
                    </li>
                    <li>
                        <a onClick={() => setType('/columns')}>Column</a>
                    </li>
                    <li>
                        <a onClick={() => setType('/grid')}>Grid</a>
                    </li>
                </ul>
            </nav>
            {(() => {
            ...
            {process.env.NODE_ENV === 'development' ? (
                <p>
                    <strong>Notice:</strong> You are currently running React in development mode. Rendering performance will be slightly degraded
