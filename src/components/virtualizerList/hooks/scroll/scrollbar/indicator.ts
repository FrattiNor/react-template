...
import { style, EventEmitter, between, getNow, EventRegister } from '@better-scroll/shared-utils';
import { Probe } from '@better-scroll/shared-utils/src/enums';
...
export default class Indicator {
    ...
    private handleHooks() {
        ...
        this.registerHooks(translaterHooks, translaterHooks.eventTypes.translate, (pos: { x: number; y: number }) => {
    ...
    private calculateclickOffsetPos(e: MouseEvent) {
        ...
        const scrollToWhere = clickPointOffset < this.currentPos ? ScrollTo.Up : ScrollTo.Down;
        let delta = 0;
    ...
    refresh() {
        ...
        if (this.canScroll(scroll[hasScrollKey])) {
            let { wrapperSize: wrapperSizeKey, scrollerSize: scrollerSizeKey, maxScrollPos: maxScrollPosKey } = this.keysMap;
    ...
    private refreshScrollInfo(wrapperSize: number, scrollerSize: number, maxScrollPos: number, indicatorElSize: number): ScrollInfo {

        // let baseSize = Math.max(Math.round((wrapperSize * wrapperSize) / (scrollerSize || wrapperSize || 1)), this.options.minSize);
        let baseSize = Math.min(
            wrapperSize,
            Math.max(Math.round((wrapperSize * wrapperSize) / (scrollerSize || wrapperSize || 1)), this.options.minSize),
        );
        ...
        const maxIndicatorScrollPos = Math.max(wrapperSize - baseSize, 0);
    ...
    private caculatePosAndSize(point: TranslaterPoint, scrollInfo: ScrollInfo): { pos: number; size: number } {
    ...
    private refreshStyle(size: number, pos: number) {
        const { translateProperty: translatePropertyKey, size: sizeKey } = this.keysMap;
        const translateZ = this.scroll.options.translateZ;

        this.indicatorEl.style[sizeKey] = `${size}px`;

        this.indicatorEl.style[style.transform as any] = `${translatePropertyKey}(${pos}px)${translateZ}`;
    }
    ...
    private indicatorNotMoved(delta: number): boolean {
        ...
        const notMoved = (currentPos === minScrollPos && delta <= 0) || (currentPos === maxScrollPos && delta >= 0);
        return notMoved;
    }

    private syncBScroll(newPos: number) {
        const timestamp = getNow();
        const { x, y, options, scroller, maxScrollY, minScrollY, maxScrollX, minScrollX } = this.scroll;
    ...
    private newPos(currentPos: number, delta: number, scrollInfo: ScrollInfo): number {
