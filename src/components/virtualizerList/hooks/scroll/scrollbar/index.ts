...
import Indicator, { IndicatorOptions, IndicatorDirection, OffsetType } from './indicator';
...
export default class ScrollBar {
    ...
    private handleOptions() {
        const userOptions = (this.scroll.options.scrollbar === true ? {} : this.scroll.options.scrollbar) as Partial<ScrollbarConfig>;
    ...
    private createIndicators() {
        ...
        const indicatorDirections = [IndicatorDirection.Horizontal, IndicatorDirection.Vertical];
        ...
        for (let i = 0; i < scrollDirectionConfigKeys.length; i++) {
            ...
            if (scroll.options[key]) {
                ...
                let isCustom = false;
                let scrollbarWrapper = customElement ? customElement : this.createScrollbarElement(direction);
                // internal scrollbar
    ...
    private createScrollbarElement(direction: IndicatorDirection, scrollbarTrackClickable = this.options.scrollbarTrackClickable) {
